...
saveRDS(regions, file="ranges.rds")
saveRDS(regionsGISD, file="rangesGISD.rds")
combined_list<-mapply(c,regions, regionsGISD, SIMPLIFY=FALSE)
names(combined_list)<-unique(invacost$Species)
saveRDS(combined_list, file="species_country_list.rds")
# 
# regions<-readRDS('~/Downloads/ranges.rds')
# regionsGISD<-readRDS('~/Downloads/rangesGISD.rds')
##Associate with continents
 species_list<-data.frame(matrix(NA, nrow=length(unique(invacost$Species)), ncol=length(unique(unlist(regions)))+1))
...
write.csv(species_list, file="species_country_origins.csv", row.names=F)

species_list<-read.csv('~/Downloads/species_country_origins.csv')
species_list[,1]<-unique(invacost$Species)
missing<-unique(invacost$Species)[which(rowSums(species_list[,2:231],na.rm=T)==0)]
write.csv(missing, file="missing_country_species.csv", row.names=F)
write.csv(species_list, file="initial_species_country.csv", row.names=F)
listform<-readRDS('~/Downloads/species_country_list.rds')
...
write.csv(out, "./output/species-list.csv", row.names = F)
saveRDS(spl, "./output/speciesRanges.rds")
...
library(rgbif)
missing<-species_list$Species[which(rowSums(species_list[,2:231], na.rm=T)==0)]
newspp<-list()
...
combined_list[which(species_list$Species%in% names(newspp_code))]<-newspp_code
...
write.csv(species_list[which(rowSums(species_list[,2:231], na.rm=T)>0),], file="species_country_origins_full.csv", row.names=F)
write.csv(species_list$Species[which(rowSums(species_list[,2:231], na.rm=T)==0)], file="species_country_missing.csv", row.names=F)
saveRDS(combined_list, file="species_country_list.rds")

colSums(species_list[,2:231], na.rm=T)[order(desc(colSums(species_list[,2:231], na.rm=T)))]
