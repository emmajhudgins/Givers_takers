...
species_list$country_written[i]<-paste(countrycode(strsplit(species_list$Countries[i], split=";")[[1]], 'iso3c', 'country.name'), collapse=";")
}
species_list$continent<-NA
for (i in 1:length(which(species_list$Species%in% names(newspp_code)))){
  species_list$continent[which(species_list$Species%in% names(newspp_code))][i]<-paste(newspp_continent[[i]],collapse=";")}
species_list$continent<-gsub('NA;', '', species_list$continent)
species_list$continent<-gsub('NA$', '', species_list$continent)
